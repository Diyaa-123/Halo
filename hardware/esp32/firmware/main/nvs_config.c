...
void nvs_config_load(nvs_config_t *cfg) {
  ...
  if (nvs_get_u8(handle, "hop_count", &hop_count_val) == ESP_OK) {
    if (hop_count_val >= 1 && hop_count_val <= NVS_CFG_HOP_MAX) {
      ...
      ESP_LOGI(TAG, "NVS override: hop_count=%u",
               (unsigned)cfg->channel_hop_count);
    } else {
  ...
  if (nvs_get_blob(handle, "chan_list", ch_blob, &len) == ESP_OK && len > 0) {
    uint8_t count =
        (len < cfg->channel_hop_count) ? (uint8_t)len : cfg->channel_hop_count;
    for (uint8_t i = 0; i < count; i++) {
      cfg->channel_list[i] = ch_blob[i];
    }
    ESP_LOGI(TAG, "NVS override: chan_list loaded (%u channels)",
             (unsigned)count);
  }
  ...
  if (nvs_get_u32(handle, "dwell_ms", &dwell_val) == ESP_OK) {
    if (dwell_val >= 10) {
      ...
      ESP_LOGW(TAG, "NVS dwell_ms=%lu too small, ignored",
               (unsigned long)dwell_val);
    }
  }
  ...
  if (nvs_get_u8(handle, "tdm_slot", &slot_val) == ESP_OK) {
    ...
    ESP_LOGI(TAG, "NVS override: tdm_slot_index=%u",
  ...
  if (nvs_get_u8(handle, "tdm_nodes", &tdm_nodes_val) == ESP_OK) {
    if (tdm_nodes_val >= 1) {
      ...
      ESP_LOGI(TAG, "NVS override: tdm_node_count=%u",
               (unsigned)cfg->tdm_node_count);
    } else {
      ESP_LOGW(TAG, "NVS tdm_nodes=%u invalid, ignored",
               (unsigned)tdm_nodes_val);
    }
  }
  ...
  if (nvs_get_u16(handle, "vital_int", &vital_int_val) == ESP_OK) {
    if (vital_int_val >= 100) {
      ...
      ESP_LOGI(TAG, "NVS override: vital_interval_ms=%u",
               cfg->vital_interval_ms);
    }
  }
  ...
  if (nvs_get_u8(handle, "wasm_max", &wasm_max_val) == ESP_OK) {
    if (wasm_max_val >= 1 && wasm_max_val <= 8) {
      ...
      ESP_LOGI(TAG, "NVS override: wasm_max_modules=%u",
               (unsigned)cfg->wasm_max_modules);
    }
  }
  ...
  if (nvs_get_blob(handle, "wasm_pubkey", cfg->wasm_pubkey, &pubkey_len) ==
          ESP_OK &&
      pubkey_len == 32) {
    cfg->wasm_pubkey_valid = 1;
    ESP_LOGI(TAG, "NVS: wasm_pubkey loaded (%02x%02x...%02x%02x)",
             cfg->wasm_pubkey[0], cfg->wasm_pubkey[1], cfg->wasm_pubkey[30],
             cfg->wasm_pubkey[31]);
  } else if (cfg->wasm_verify) {
    ESP_LOGW(
        TAG,
        "wasm_verify=1 but no wasm_pubkey in NVS — uploads will be rejected");
  }

  /* ADR-060: CSI channel override. */
  uint8_t csi_ch_val;
  if (nvs_get_u8(handle, "csi_channel", &csi_ch_val) == ESP_OK) {
    if ((csi_ch_val >= 1 && csi_ch_val <= 14) ||
        (csi_ch_val >= 36 && csi_ch_val <= 177)) {
      cfg->csi_channel = csi_ch_val;
      ESP_LOGI(TAG, "NVS override: csi_channel=%u", (unsigned)cfg->csi_channel);
    } else {
      ESP_LOGW(TAG, "NVS csi_channel=%u invalid, ignored",
               (unsigned)csi_ch_val);
    }
  }
  ...
  size_t mac_len = 6;
  if (nvs_get_blob(handle, "filter_mac", cfg->filter_mac, &mac_len) == ESP_OK &&
      mac_len == 6) {
