...
export default function DigitalTwin({ patientState = 'stable', setPatientState, healthScore = 82, selectedOccupantIndex = 0 }) {
  ...
  const liveBodyRegions = bodyRegions.map(r => {
    ...
    if (r.id === 'left-leg' || r.id === 'right-leg') {
      if (sensing.harPrediction) {
        const gaitRisk = sensing.harPrediction === 'fall' ? 'High' : sensing.harPrediction === 'sit' || sensing.harPrediction === 'sitdown' ? 'Low' : 'Moderate';
        const gaitStat = sensing.harPrediction === 'fall' ? 'critical' : 'stable';
        return { ...r, status: gaitStat, details: { metric: 'Gait Activity', value: sensing.harPrediction.toUpperCase(), risk: gaitRisk, recommendation: sensing.harPrediction === 'fall' ? 'Fall detected! Check immediately.' : 'Live ML Gait Tracking' } };
      }
    }
  ...
  return (
    <div className={`digital-twin ${stateConf.animClass} ${isFullscreen ? 'digital-twin--fullscreen' : ''}`}>
      ...
      {/* State Status Banner & Gait Prediction */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'absolute', top: '70px', left: '50%', transform: 'translateX(-50%)', zIndex: 10, gap: '6px' }}>
        <div className={`digital-twin__state-banner ${effectiveState === 'respiratory_distress' ? 'digital-twin__state-banner--critical' : ''}`} style={{ position: 'relative', top: 'auto', left: 'auto', transform: 'none' }}>
        ...
        {sensing.harPrediction && (
          <div className={`badge badge-${sensing.harPrediction === 'fall' ? 'critical' : 'stable'}`} style={{ backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.4)', padding: '6px 16px', background: sensing.harPrediction === 'fall' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(78, 222, 163, 0.2)', fontSize: '11px', letterSpacing: '0.05em' }}>
            <span className="material-icons icon-sm">
              {sensing.harPrediction === 'walk' ? 'directions_walk' : sensing.harPrediction.includes('sit') ? 'chair' : sensing.harPrediction === 'fall' ? 'warning' : 'accessibility_new'}
            </span>
            GAIT: {sensing.harPrediction.toUpperCase()} {sensing.harConfidence ? `(${(sensing.harConfidence * 100).toFixed(0)}%)` : ''}
          </div>
        )}
      </div>
