...
function AnimatedOccupant({ index, sensing, twinColor, isPrimary }) {
  ...
  useFrame((state, delta) => {
    ...
    const isMoving = sensing.motionLevel === 'active';
    const br = sensing.breathingRate || 15;
    
    // 1. Determine Target Position based on LIVE FEED
    const radius = isPrimary ? estimatedDistance : estimatedDistance + (index * 0.5);
    const targetX = Math.cos(estimatedAngle) * radius;
    const targetZ = Math.sin(estimatedAngle) * radius;
    
    // 2. Smoothly move (lerp) the twin to the target X,Z
    const moveSpeed = isMoving ? 2.0 * delta : 0.8 * delta;
  ...
  return (
    <group ref={groupRef} position={[0,0,0]}>
      <HumanTwinModel
        color={isPrimary ? twinColor : '#8d8a84'}
        wireframe={true}
