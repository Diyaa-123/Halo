...
BW = 80;                % bandwidth
FILE = './mesitting.pcap';% capture file
...
while (k <= n)
    ...
    if(sum(csi_mag(k,:)) > 0)
        csi_mag_clean(index,:) = csi_mag(k,:);
        ts(1,index)=timestampms(1,k);
        index = index + 1;
    end
    k = k + 1;
end
csiDenoised = zeros([rowNumbers, sizeSub]);
...
% 
 colormap summer
 shading interp
 csi_buff2 = fftshift(csi_buff,2);
 csi_phase = rad2deg(angle(csi_buff));
 s1 = zeros (349,128);
% for i = 1:349
%     s1(i,:) = csi_mag(3*(i-1)+1, :);
% end
% figure
% surf(s1)
%  fOffset = wlanCoarseCFOEstimate(csi_mag,BW)
 
 %plotcsi(csi_buff, NFFT, false)
%[coeff,score,latent,tsquared,explained,mu] = pca(csi_mag);
%pcs = score(:,1:3);
% figure;
% plot(csi_mag(:,100))
% hold on
...
%  yCalc2 = X*b;
%  plot(x,yCalc2);
% detrending = csi_mag(:,100) - yCalc2;
% 
% % plot(x, detrending);
% % hold on;
% 
% t = transpose(1:151)                                             % Time (s)
% t = t*0.01;
% 
% 
% a = csi_mag(:,100);                                    % Acceleration (g)
% L = length(t);
...
% fastfour=fft(csi_mag(:,100));
% figure
% plot(fastfour(:,1),'o');

% figure;
% fourtr = fft(detrending);
%plot(fourtr,x);