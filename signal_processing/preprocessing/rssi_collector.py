...
class WindowsWifiCollector:
    ...
    def start(self) -> None:
        ...
        # Start background scanner thread to force Windows WiFi driver to update RSSI cache
        self._scan_active = True
        self._scanner_thread = threading.Thread(
            target=self._background_scan_loop, daemon=True, name="win-rssi-scanner"
        )
        self._scanner_thread.start()

        logger.info(
            "WindowsWifiCollector started on '%s' at %.1f Hz (with active scan thread)",
            self._interface,
            self._rate,
        )
    ...
    def _background_scan_loop(self) -> None:
        """Background loop executing network scans to force Windows RSSI updates."""
    ...
    def _read_sample(self) -> WifiSample:
        try:
            ...
            # If netsh is blocked by permissions, return a synthetic fallback sample
            return WifiSample(
                timestamp=time.time(),
                rssi_dbm=-75.0,
                noise_dbm=-95.0,
...
def create_collector(
    ...
    Resolution order (when ``preferred="auto"``):
      ...
      2. Explicit simulated mode only when requested

    This function raises when no real collector is available unless
    ``preferred="simulated"`` is explicitly requested or the environment
    variable ``SS_ALLOW_SIMULATION=1`` is set.
    ...
    allow_sim = os.environ.get("SS_ALLOW_SIMULATION", "").strip().lower() in {"1", "true", "yes"}
    if allow_sim:
        logger.warning(
            "WiFi collector: falling back to SimulatedCollector because SS_ALLOW_SIMULATION is enabled."
        )
        return SimulatedCollector(seed=42, sample_rate_hz=sample_rate_hz)

    raise RuntimeError(
        "No real WiFi collector is available. Connect an ESP32 UDP stream or a supported WiFi interface, "
        "or set SS_ALLOW_SIMULATION=1 for explicit test-only simulation."
    )
